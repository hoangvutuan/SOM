...
<body>
	<h3><span class="label label-default"><spring:message code="student.profile" /></span></h3>
	<div class="container">
		<table class="table table-hover">
			...
			<tbody>
					...
					<tr>					
						...
						<td>
						<a href='javascript:editStudent(${student.id})'><span class="glyphicon glyphicon-pencil"></span> </a>
						</td>
					</tr>
			</tbody>
		</table>
	...
	<br/>
	<h3><span class="label label-default"><spring:message code="student.transcript" /></span></h3>
	<div class="container">
		<table class="table table-hover">
			<thead>
				<tr>
			...
			<tbody>
			<c:forEach items="${studentsections}" var="studentsection" varStatus="loop">
				<tr>					
					<td>${loop.index + 1}</td>
					<td>${studentsection.section.name}</td>
					<td>${studentsection.section.faculty.firstName} ${studentsection.section.faculty.lastName}</td>
					<td><fmt:formatDate pattern="MM/dd/yyyy" value="${studentsection.section.block.startDate}" /></td>
					<c:choose>
   	...
   	<script>
		...
		var showStudentDialog = function(action, userid) {
			if (action == 'edit') {
				ajaxUrl = "/mumsched/editstudentprofile/" + userid; 
			}
			$.ajax({
				type : "GET",
				url : ajaxUrl,
				success : function(data) {
					var facultyDialog = $("#editStudentDialog");
