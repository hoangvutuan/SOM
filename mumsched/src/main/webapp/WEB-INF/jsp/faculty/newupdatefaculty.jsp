		...
		<form:input type="hidden" path="id" id="id" />
			<div class="form-group">
				...
				<form:select path="user" class="form-control">
					<c:if test="${edit}">
						<form:option value="${faculty.user.username}" selected="true">${faculty.user.username}</form:option>
					</c:if>
				
					<c:forEach items="${availableUsers}" var="useritem">
						<%-- <c:choose>
							<c:when test="${useritem.username eq faculty.user.username}">
								<form:option value="${useritem.username}" selected="true">${useritem.username}</form:option>
							</c:when>
							<c:otherwise> --%>
			...
			<div class = "form-group">
				...
				<form:select path="targetBlock" id="targetBlock"
					...
					<c:forTokens items="${blockNames}" delims="," var="blockItem">
						<c:set var="checkB" value="${fn:indexOf(faculty.targetBlock, blockItem)}"/>
